...
class Table {
    ...
    deleteSelected() {
        ...
        if (this.deleteSelectedSelector) {
            ...
            this.deleteSelectedSelector.addEventListener('click', (e) => {
                ...
                const selectedIds = selectedRows.map(row => row.querySelector('input[type="checkbox"]').value);
                const bulkDeleteUrl = this.deleteSelectedSelector.dataset.bulkDeleteUrl;
                                ...
                                if (bulkDeleteUrl) {
                                    fetch(bulkDeleteUrl, {
                                        ...
                                        if (response.ok) {
                                            ...
                                            const remainingRows = this.rows.filter(row => !selectedRows.includes(row));
                                    ...
                                    // Fallback for non-AJAX implementation or missing route
                                    selectedRows.forEach(row => row.remove());
    ...
    deleteRow() {
        ...
        if (this.tbody) {
            this.tbody.addEventListener('click', (e) => {
                const button = e.target.closest(this.parentInstance.deleteRowSelector);
                if (!button) return;

                e.preventDefault();
                const deleteUrl = button.dataset.deleteUrl;

                this.alert(this.deleteRowMessage).then((result) => {
                    if (result.isConfirmed) {
                        ...
                        if (deleteUrl) {
                            ...
                            fetch(deleteUrl, {
                                ...
                                if (response.ok) {
                                    console.log('Delete successful, removing row from DOM');
                                    rowToBeDelete.remove();
                                    this.updateDatasetAfterDelete(rowToBeDelete);
                            ...
                            // Fallback for client-side only (if no URL provided)
                            rowToBeDelete.remove();
                            this.updateDatasetAfterDelete(rowToBeDelete);
    ...
    updateDatasetAfterDelete(deletedRow) {
        // Remove from this.rows
        const rowIndex = this.rows.indexOf(deletedRow);
        console.log('Row index in this.rows:', rowIndex);

        if (rowIndex > -1) {
            this.rows.splice(rowIndex, 1);
        } else {
            console.warn('Row NOT found in this.rows! Attempting to match by ID...');
            // Fallback: This indicates the DOM element is different from what we stored.
            // But we can try to find the row by unique content if needed.
        }

        // Remove from this.filteredRows
        const filteredIndex = this.filteredRows.indexOf(deletedRow);
        console.log('Row index in this.filteredRows:', filteredIndex);
    ...
    alert(message) {
        return Swal.fire({
            title: 'Are you sure?',
            text: message ?? this.deleteRowMessage,
            icon: 'warning',
