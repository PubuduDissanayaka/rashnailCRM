<div class="tab-pane active" id="business-tab" role="tabpanel">
    <form id="business-form" action="{{ route('settings.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-12">
                <h5 class="mt-3 mb-2">Business Hours</h5>

                <div class="alert alert-info">
                    <i class="ti ti-info-circle me-2"></i>
                    This weekly schedule controls both staff attendance tracking and when customers can book appointments — appointments outside these hours, or that would run past closing, will be rejected.
                </div>
                
                @php
                    // Load business hours from BusinessHoursService (the single
                    // source of truth shared by attendance AND appointment booking).
                    $bhs = app(\App\Services\BusinessHoursService::class);
                    $hoursConfig = $bhs->getConfig();

                    // Top-level fields (grace period, etc.)
                    $gracePeriodMinutes      = $hoursConfig['grace_period_minutes'] ?? 15;
                    $overtimeAfterHours      = $hoursConfig['overtime_start_after_hours'] ?? 1;
                    $minimumShiftHours       = $hoursConfig['minimum_shift_hours'] ?? 4;
                    $maximumShiftHours       = $hoursConfig['maximum_shift_hours'] ?? 12;
                    $halfDayThresholdHours   = $hoursConfig['half_day_threshold_hours'] ?? 4;

                    // Day-level schedule — may be stored under 'weekdays' (from
                    // form submission) or 'business_hours' (from defaults / compat).
                    $weekdayData = $hoursConfig['weekdays'] ?? $hoursConfig['business_hours'] ?? [];

                    // Day defaults (used when a day is absent from saved data)
                    $dayDefaults = [
                    ...
                    // Merge saved (or default) data into the day defaults so
                    // every day always has an entry — even if it was absent
                    // from the saved JSON (e.g. unchecked days that didn't
                    // submit anything).
                    $allDays = [];
                    foreach ($dayDefaults as $dayName => $dayDefault) {
                        $saved = $weekdayData[$dayName] ?? [];
                        $allDays[$dayName] = array_merge($dayDefault, $saved);
                    }
                @endphp
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="grace_period_minutes" 
                                   ...
                                   value="{{ old('settings.attendance.business_hours.grace_period_minutes', $gracePeriodMinutes) }}"
                                   min="0" max="60">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="overtime_start_after_hours" 
                                   ...
                                   value="{{ old('settings.attendance.business_hours.overtime_start_after_hours', $overtimeAfterHours) }}"
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="minimum_shift_hours" 
                                   ...
                                   value="{{ old('settings.attendance.business_hours.minimum_shift_hours', $minimumShiftHours) }}"
                                   min="0" max="24" step="0.5">
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="maximum_shift_hours" 
                                   ...
                                   value="{{ old('settings.attendance.business_hours.maximum_shift_hours', $maximumShiftHours) }}"
                                   min="0" max="24" step="0.5">
                    ...
                    <div class="col-md-4">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="half_day_threshold_hours" 
                                   ...
                                   value="{{ old('settings.attendance.business_hours.half_day_threshold_hours', $halfDayThresholdHours) }}"
                                   min="0" max="24" step="0.5">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex align-items-center mb-2">
                            <div class="form-check me-3">
                                {{-- Hidden input ensures 'enabled=0' is sent when checkbox is unchecked --}}
                                <input type="hidden" name="settings[attendance.business_hours][weekdays][{{ $day }}][enabled]" value="0">
                                <input class="form-check-input day-enabled" type="checkbox" 
                                       id="{{ $day }}_enabled"
                                       name="settings[attendance.business_hours][weekdays][{{ $day }}][enabled]"
                                       value="1"
                                       @if($allDays[$day]['enabled'] ?? false) checked @endif>
                            ...
                            <div class="flex-grow-1 d-flex">
                                <input type="time" class="form-control me-2" 
                                       ...
                                       value="{{ $allDays[$day]['open'] ?? '09:00' }}"
                                       @if(!($allDays[$day]['enabled'] ?? false)) disabled @endif>
                                <input type="time" class="form-control" 
                                       id="{{ $day }}_close" 
                                       name="settings[attendance.business_hours][weekdays][{{ $day }}][close]"
                                       value="{{ $allDays[$day]['close'] ?? '17:00' }}"
                                       @if(!($allDays[$day]['enabled'] ?? false)) disabled @endif>
                            </div>
                        </div>
                        @endforeach
